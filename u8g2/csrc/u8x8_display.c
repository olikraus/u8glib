...
void u8x8_d_helper_display_init(u8x8_t *u8g2, const u8x8_display_info_t *display_info)
{
      ...
      /* 3) do reset */
      u8x8_gpio_SetReset(u8g2, 1);
      u8x8_gpio_Delay(u8g2, U8X8_MSG_DELAY_MILLI, u8g2->display_info->reset_pulse_width_ms);
      u8x8_gpio_SetReset(u8g2, 0);
      u8x8_gpio_Delay(u8g2, U8X8_MSG_DELAY_MILLI, u8g2->display_info->reset_pulse_width_ms);
      u8x8_gpio_SetReset(u8g2, 1);
      u8x8_gpio_Delay(u8g2, U8X8_MSG_DELAY_MILLI, u8g2->display_info->post_reset_wait_ms);
}    
...
uint8_t u8x8_display_DrawTile(u8x8_t *u8g2, uint8_t x, uint8_t y, uint8_t cnt, uint8_t *tile_ptr)
{
  ...
  return u8g2->display_cb(u8g2, U8X8_MSG_DISPLAY_DRAW_TILE, 1, (void *)&tile);
}

void u8x8_display_Init(u8x8_t *u8g2)
{
  u8g2->display_cb(u8g2, U8X8_MSG_DISPLAY_INIT, 0, NULL);  
}

void u8x8_display_SetPowerSave(u8x8_t *u8g2, uint8_t is_enable)
{
  u8g2->display_cb(u8g2, U8X8_MSG_DISPLAY_SET_POWER_SAVE, is_enable, NULL);  
}

void u8x8_display_SetFlipMode(u8x8_t *u8g2, uint8_t mode)
{
  u8g2->display_cb(u8g2, U8X8_MSG_DISPLAY_SET_FLIP_MODE, mode, NULL);  
}

void u8x8_display_SetContrast(u8x8_t *u8g2, uint8_t value)
{
  u8g2->display_cb(u8g2, U8X8_MSG_DISPLAY_SET_CONTRAST, value, NULL);  
}
...
void u8x8_display_ClearScreen(u8x8_t *u8g2)
{
  ...
  do
  {
    u8g2->display_cb(u8g2, U8X8_MSG_DISPLAY_DRAW_TILE, u8g2->display_info->tile_width, (void *)&tile);
    tile.y_pos++;
  } while( tile.y_pos < h );
}
